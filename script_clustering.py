...
def setup_logger(log_folder="logs"):
    """Sets up a logger that outputs to console and a date-versioned file."""
    # Create log folder if it doesn't exist
    if not os.path.exists(log_folder):
        os.makedirs(log_folder)

    now = datetime.now()
    log_filename = now.strftime(f"{log_folder}/xcluster_run_%Y-%m-%d_%H-%M-%S.log")

    logger = logging.getLogger("XCluster")
    logger.setLevel(logging.INFO) # Set the global logging level

    # Prevent multiple handlers if setup_logger is called more than once (e.g., in interactive sessions)
    ...
    ch.setLevel(logging.INFO) # Output INFO and above to console
...
class CaptionBasedCriteriaProposer:
    ...
    def run(self, image_paths):
        ...
        if not initial_captions:
            logger.error("CriteriaProposer: Failed to generate initial captions.")
            return []
        ...
        if not proposed_criteria:
            logger.error("CriteriaProposer: Failed to propose criteria from captions.")
...
def export_results_to_csv(image_files_list, 
    ...
    granularity_levels = ['coarse', 'middle', 'fine'] # As used in SemanticGrouper
            ...
            for cluster_name, images_in_cluster in clusters.items():
                for img_path in images_in_cluster:
                    if img_path not in image_to_cluster_lookup:
                        image_to_cluster_lookup[img_path] = {} # Should already exist from init
        ...
        with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for img_path in image_files_list:
                ...
                for criterion in sorted_criteria: # Iterate in sorted order for consistency
                    ...
                    for gran_level in granularity_levels:
                        cluster_name = criterion_clusters.get(gran_level, "N/A")
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(IMAGE_ROOT_DIRECTORY):
        logger.error(f"Error: Image root directory '{IMAGE_ROOT_DIRECTORY}' not found.")
        exit()

    all_image_files = []
    logger.info(f"Scanning for images in '{IMAGE_ROOT_DIRECTORY}' and its subdirectories...")
    ...
    if not all_image_files:
        logger.error(f"No images found in '{IMAGE_ROOT_DIRECTORY}' or its subdirectories. Please check the path and add some images.")
        exit()
    ...
    if len(all_image_files) > SUBSET_SIZE:
        ...
        logger.info(f"Selected a random subset of {SUBSET_SIZE} images for processing.")
    else:
        image_files = all_image_files
        logger.info(f"Using all {len(image_files)} found images (less than {SUBSET_SIZE}).")
    ...
    if discovered_criteria:
        ...
        logger.warning("No criteria were discovered. CSV export will be limited.")
        ...
        export_results_to_csv(image_files, initial_captions_dict, {}, [], "xcluster_output_no_criteria.csv")
        exit()
    ...
    if all_cluster_assignments:
        for criterion, assignments_by_granularity in all_cluster_assignments.items():
            ...
            for granularity_level, clusters in assignments_by_granularity.items():
                logger.info(f"Granularity: {granularity_level} - Found {len(clusters)} clusters.")
                if clusters:
                    first_cluster_name = list(clusters.keys())[0]
                    num_images_in_first_cluster = len(clusters[first_cluster_name])
                    logger.info(f"Example Cluster: \"{first_cluster_name}\" ({num_images_in_first_cluster} images)")
    else:
